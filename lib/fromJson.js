...
function fromJson(json, handler) {
  ...
  switch (type) {
    ...
    case 'number':
      return json;
    case 'string':
      if (json.substr(0, 13) === "_vdom_as_json"){
        matches = json.match(vdomAsJsonFunctionIndexRegex);
        if (matches){
          return function(e){ handler(matches[1], matches[2], e); };
        }
      }
      return json;
  }
