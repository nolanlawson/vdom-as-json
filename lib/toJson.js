...
function serializeFunction(res, functionIndex, key, func){
  var index
    , r = (functionIndex.serializeProp || defaultSerializeFunctionProp)(key, func)
    , ev = r[0]
    , serialized = r[1];
  ...
  res[ev] = "_vdom_as_json_f_i#" + index + "@" + serialized;
}
...
function plainObjectToJson(obj, functionIndex) {
  ...
  for (var key in obj) {
    ...
    if (type === "undefined"){
      ...
      serializeFunction(res, functionIndex, key, val);
    } else {
